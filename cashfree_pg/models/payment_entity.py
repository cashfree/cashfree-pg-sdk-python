...
class PaymentEntity(BaseModel):
    ...
    PaymentEntity
    """
    cf_payment_id: Optional[StrictStr] = Field(default=None, description="Payment entity full object.")
    ...
    order_amount: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Order amount can be different from payment amount if you collect service fee from the customer.")
    payment_group: Optional[StrictStr] = Field(default=None, description="Type of payment group. One of ['prepaid_card', 'upi_ppi_offline', 'cash', 'upi_credit_card', 'paypal', 'net_banking', 'cardless_emi', 'credit_card', 'bank_transfer', 'pay_later', 'debit_card_emi', 'debit_card', 'wallet', 'upi_ppi', 'upi', 'credit_card_emi'].")
    payment_currency: Optional[StrictStr] = None
    payment_amount: Optional[Union[StrictFloat, StrictInt]] = None
    payment_time: Optional[StrictStr] = Field(default=None, description="This is the time when the payment was initiated.")
    payment_completion_time: Optional[StrictStr] = Field(default=None, description="This is the time when the payment reaches its terminal state.")
    payment_status: Optional[StrictStr] = Field(default=None, description="The transaction status can be one of  [\"SUCCESS\", \"NOT_ATTEMPTED\", \"FAILED\", \"USER_DROPPED\", \"VOID\", \"CANCELLED\", \"PENDING\"].")
    payment_message: Optional[StrictStr] = None
    bank_reference: Optional[StrictStr] = Field(default=None, description="Issuing bank’s transaction reference number.")
    auth_id: Optional[StrictStr] = Field(default=None, description="Authorisation ID provided by the issuing bank.")
    ...
    international_payment: Optional[InternationalPaymentEntity] = None
    payment_gateway_details: Optional[PaymentGatewayDetails] = None
    payment_surcharge: Optional[PaymentEntityPaymentSurcharge] = None
    __properties = ["cf_payment_id", "order_id", "entity", "error_details", "is_captured", "order_amount", "payment_group", "payment_currency", "payment_amount", "payment_time", "payment_completion_time", "payment_status", "payment_message", "bank_reference", "auth_id", "order_currency", "authorization", "payment_method", "international_payment", "payment_gateway_details", "payment_surcharge"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> PaymentEntity:
        ...
        if "cf_payment_id, order_id, entity, error_details, is_captured, order_amount, payment_group, payment_currency, payment_amount, payment_time, payment_completion_time, payment_status, payment_message, bank_reference, auth_id, order_currency, authorization, payment_method, international_payment, payment_gateway_details, payment_surcharge" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
    ...
    def to_dict(self):
        ...
        if self.international_payment:
            _dict['international_payment'] = self.international_payment.to_dict()
        ...
        # override the default output from pydantic by calling `to_dict()` of payment_surcharge
        if self.payment_surcharge:
            _dict['payment_surcharge'] = self.payment_surcharge.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: dict) -> PaymentEntity:
        ...
        _obj = PaymentEntity.parse_obj({
            ...
            "international_payment": InternationalPaymentEntity.from_dict(obj.get("international_payment")) if obj.get("international_payment") is not None else None,
            "payment_gateway_details": PaymentGatewayDetails.from_dict(obj.get("payment_gateway_details")) if obj.get("payment_gateway_details") is not None else None,
            "payment_surcharge": PaymentEntityPaymentSurcharge.from_dict(obj.get("payment_surcharge")) if obj.get("payment_surcharge") is not None else None
        })
        return _obj


# Pydantic v2: resolve forward references & Annotated types
PaymentEntity.model_rebuild()
