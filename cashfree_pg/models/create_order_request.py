...
class CreateOrderRequest(BaseModel):
    ...
    order_amount: Union[Annotated[float, Field(strict=True, ge=1)], Annotated[int, Field(strict=True, ge=1)]] = Field(description="Bill amount for the order. Provide upto two decimals. 10.15 means Rs 10 and 15 paisa. For orders in non-INR currency, please refer to [supported amounts](https://www.cashfree.com/docs/payments/international-payments/ipg/currencies-supported#decimal-support) per currency.")
    order_currency: StrictStr = Field(description="Currency for the order. INR if left empty. For support currency list, refer [here](https://www.cashfree.com/docs/payments/international-payments/ipg/currencies-supported#full-currency-list). Submit [Support Form](https://merchant.cashfree.com/auth/login) to enable new currencies.")
    ...
    __properties = ["order_id", "order_amount", "order_currency", "cart_details", "customer_details", "terminal", "order_meta", "order_expiry_time", "order_note", "order_tags", "order_splits", "products"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> CreateOrderRequest:
        ...
        if "order_id, order_amount, order_currency, cart_details, customer_details, terminal, order_meta, order_expiry_time, order_note, order_tags, order_splits, products" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
