...
class OrderEntity(BaseModel):
    ...
    OrderEntity
    """
    cf_order_id: Optional[StrictStr] = Field(default=None, description="unique id generated by cashfree for your order.")
    order_id: Optional[StrictStr] = Field(default=None, description="order_id sent during the api request.")
    entity: Optional[StrictStr] = Field(default=None, description="Type of the entity.")
    order_currency: Optional[StrictStr] = Field(default=None, description="Currency of the order. Example INR.")
    order_amount: Optional[Union[StrictFloat, StrictInt]] = None
    order_status: Optional[StrictStr] = Field(default=None, description="Possible values are  - `ACTIVE`: Order does not have a sucessful transaction yet - `PAID`: Order is PAID with one successful transaction - `EXPIRED`: Order was not PAID and not it has expired. No transaction can be initiated for an EXPIRED order. `TERMINATED`: Order terminated `TERMINATION_REQUESTED`: Order termination requested.")
    payment_session_id: Optional[StrictStr] = None
    order_expiry_time: Optional[StrictStr] = None
    order_note: Optional[StrictStr] = Field(default=None, description="Additional note for order.")
    created_at: Optional[StrictStr] = Field(default=None, description="When the order was created at cashfree's server.")
    ...
    order_tags: Optional[Dict[str, Annotated[str, Field(min_length=1, strict=True, max_length=255)]]] = Field(default=None, description="Custom Tags in the form of {\"key\":\"value\"} which can be passed for an order. A maximum of 10 tags can be added.")
    cart_details: Optional[CartDetailsEntity] = None
    terminal_data: Optional[TerminalData] = None
    products: Optional[OrderEntityProducts] = None
    __properties = ["cf_order_id", "order_id", "entity", "order_currency", "order_amount", "order_status", "payment_session_id", "order_expiry_time", "order_note", "created_at", "order_splits", "customer_details", "order_meta", "order_tags", "cart_details", "terminal_data", "products"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> OrderEntity:
        ...
        if "cf_order_id, order_id, entity, order_currency, order_amount, order_status, payment_session_id, order_expiry_time, order_note, created_at, order_splits, customer_details, order_meta, order_tags, cart_details, terminal_data, products" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of terminal_data
        if self.terminal_data:
            _dict['terminal_data'] = self.terminal_data.to_dict()
    ...
    def from_dict(cls, obj: dict) -> OrderEntity:
        ...
        _obj = OrderEntity.parse_obj({
            ...
            "cart_details": CartDetailsEntity.from_dict(obj.get("cart_details")) if obj.get("cart_details") is not None else None,
            "terminal_data": TerminalData.from_dict(obj.get("terminal_data")) if obj.get("terminal_data") is not None else None,
            "products": OrderEntityProducts.from_dict(obj.get("products")) if obj.get("products") is not None else None
        })
        return _obj
