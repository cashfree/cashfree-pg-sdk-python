...
class CreateSubscriptionRefundRequest(BaseModel):
    ...
    """
    cf_payment_id: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Cashfree subscription payment reference number.")
    subscription_id: StrictStr = Field(description="A unique ID passed by merchant for identifying the subscription.")
    payment_id: StrictStr = Field(description="A unique ID passed by merchant for identifying the transaction.")
    ...
    refund_note: Optional[StrictStr] = Field(default=None, description="Refund note for merchant reference. To simulate refund status in Sandbox, pass SUCCESS, FAILED, PENDING, or ACTIVE in the refund_note field. This is a case-sensitive parameter.")
    refund_speed: Optional[StrictStr] = Field(default=None, description="Refund speed. Can be INSTANT or STANDARD. UPI supports only STANDARD refunds, Enach and Pnach supports only INSTANT refunds.")
    __properties = ["cf_payment_id", "subscription_id", "payment_id", "refund_id", "refund_amount", "refund_note", "refund_speed"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> CreateSubscriptionRefundRequest:
        ...
        if "cf_payment_id, subscription_id, payment_id, refund_id, refund_amount, refund_note, refund_speed" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
...
CreateSubscriptionRefundRequest.model_rebuild()
