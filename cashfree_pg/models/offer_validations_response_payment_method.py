...
OFFERVALIDATIONSRESPONSEPAYMENTMETHOD_ONE_OF_SCHEMAS = ["OfferAll", "OfferCard", "OfferEMI", "OfferNB", "OfferPaylater", "OfferUPI", "OfferWallet"]

class OfferValidationsResponsePaymentMethod(BaseModel):
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    one_of_schemas: List[str] = Literal[OFFERVALIDATIONSRESPONSEPAYMENTMETHOD_ONE_OF_SCHEMAS]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = OfferValidationsResponsePaymentMethod.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OfferValidationsResponsePaymentMethod with oneOf schemas: OfferAll, OfferCard, OfferEMI, OfferNB, OfferPaylater, OfferUPI, OfferWallet. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OfferValidationsResponsePaymentMethod with oneOf schemas: OfferAll, OfferCard, OfferEMI, OfferNB, OfferPaylater, OfferUPI, OfferWallet. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> OfferValidationsResponsePaymentMethod:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> OfferValidationsResponsePaymentMethod:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OfferValidationsResponsePaymentMethod with oneOf schemas: OfferAll, OfferCard, OfferEMI, OfferNB, OfferPaylater, OfferUPI, OfferWallet. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OfferValidationsResponsePaymentMethod with oneOf schemas: OfferAll, OfferCard, OfferEMI, OfferNB, OfferPaylater, OfferUPI, OfferWallet. Details: " + ", ".join(error_messages))
        else:
            return instance
...
OfferValidationsResponsePaymentMethod.model_rebuild()
