...
class CreateLinkRequest(BaseModel):
    ...
    CreateLinkRequest
    """
    link_id: Optional[Annotated[str, Field(strict=True, max_length=50)]] = Field(default=None, description="Unique Identifier (provided by merchant) for the Link. Alphanumeric and only - and _ allowed (50 character limit). Use this for other link-related APIs.")
    ...
    link_currency: Annotated[str, Field(strict=True)] = Field(description="Currency for the payment link. Default is INR. Submit [Support Form](https://merchant.cashfree.com/auth/login) to enable new currencies.")
    link_purpose: Optional[Annotated[str, Field(strict=True, max_length=500)]] = Field(default=None, description="A brief description for which payment must be collected. This is shown to the customer.")
    customer_details: LinkCustomerDetailsEntity
    link_partial_payments: Optional[StrictBool] = Field(default=None, description="If \"true\", customer can make partial payments for the link.")
    link_minimum_partial_amount: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Minimum amount in first installment that needs to be paid by the customer if partial payments are enabled. This should be less than the link_amount.")
    link_expiry_time: Optional[StrictStr] = Field(default=None, description="Time after which the link expires. Customers will not be able to make the payment beyond the time specified here. You can provide them in a valid ISO 8601 time format. Default is 30 days.")
    link_notify: Optional[LinkNotifyEntity] = None
    link_auto_reminders: Optional[StrictBool] = Field(default=None, description="If \"true\", reminders will be sent to customers for collecting payments.")
    ...
    enable_invoice: Optional[StrictBool] = Field(default=None, description="Enable or disable invoice generation for this payment link.")
    subscription: Optional[Subscription] = None
    __properties = ["link_id", "link_amount", "link_currency", "link_purpose", "customer_details", "link_partial_payments", "link_minimum_partial_amount", "link_expiry_time", "link_notify", "link_auto_reminders", "link_notes", "link_meta", "order_splits", "enable_invoice", "subscription"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> CreateLinkRequest:
        ...
        if "link_id, link_amount, link_currency, link_purpose, customer_details, link_partial_payments, link_minimum_partial_amount, link_expiry_time, link_notify, link_auto_reminders, link_notes, link_meta, order_splits, enable_invoice, subscription" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
    ...
    def from_dict(cls, obj: dict) -> CreateLinkRequest:
        ...
        _obj = CreateLinkRequest.parse_obj({
            ...
            "enable_invoice": obj.get("enable_invoice"),
...
CreateLinkRequest.model_rebuild()
