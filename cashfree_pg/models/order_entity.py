...
class OrderEntity(BaseModel):
    ...
    order_status: Optional[StrictStr] = Field(None, description="Possible values are  - `ACTIVE`: Order does not have a sucessful transaction yet - `PAID`: Order is PAID with one successful transaction - `EXPIRED`: Order was not PAID and not it has expired. No transaction can be initiated for an EXPIRED order. `TERMINATED`: Order terminated `TERMINATION_REQUESTED`: Order termination requested")
    ...
    def from_dict(cls, obj: dict) -> OrderEntity:
        ...
        if not isinstance(obj, dict):
            return OrderEntity.model_validate(obj)

        _obj = OrderEntity.model_validate({
