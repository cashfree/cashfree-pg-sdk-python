...
class SubscriptionEntity(BaseModel):
    ...
    subscription_expiry_time: Optional[StrictStr] = Field(default=None, description="Time at which the subscription will expire. We store timestamps in IST.")
    subscription_first_charge_time: Optional[StrictStr] = Field(default=None, description="Time at which the first charge will be made for the subscription. Applicable only for PERIODIC plans. Cashfree stores timestamps in IST.")
    ...
    subscription_status: Optional[StrictStr] = Field(default=None, description="Status of the subscription. For more details, see [Subscription Lifecycle](https://www.cashfree.com/docs/payments/subscription/create#subscription-lifecycle).")
    subscription_tags: Optional[Dict[str, Any]] = Field(default=None, description="Tags for the subscription.")
    next_schedule_date: Optional[StrictStr] = Field(default=None, description="The date on which the next payment is scheduled to be processed. Cashfree stores timestamps in IST. For ON_DEMAND plans this will be null .")
    __properties = ["authorisation_details", "cf_subscription_id", "customer_details", "plan_details", "subscription_expiry_time", "subscription_first_charge_time", "subscription_id", "subscription_meta", "subscription_note", "subscription_session_id", "subscription_payment_splits", "subscription_status", "subscription_tags", "next_schedule_date"]
    ...
    def from_json_for_one_of(cls, json_str: str) -> SubscriptionEntity:
        ...
        if "authorisation_details, cf_subscription_id, customer_details, plan_details, subscription_expiry_time, subscription_first_charge_time, subscription_id, subscription_meta, subscription_note, subscription_session_id, subscription_payment_splits, subscription_status, subscription_tags, next_schedule_date" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
    ...
    def from_dict(cls, obj: dict) -> SubscriptionEntity:
        ...
        _obj = SubscriptionEntity.parse_obj({
            ...
            "next_schedule_date": obj.get("next_schedule_date")
        })
        return _obj


# Pydantic v2: resolve forward references & Annotated types
SubscriptionEntity.model_rebuild()
