...
class OrderAuthenticatePaymentRequest(BaseModel):
    ...
    """
    otp: Optional[StrictStr] = Field(default=None, description="OTP collected from the customer. Mandatory only when action is SUBMIT_OTP.")
    action: StrictStr = Field(description="Action intended for this request - can be SUBMIT_OTP or RESEND_OTP.")
    ...
    def from_json_for_one_of(cls, json_str: str) -> OrderAuthenticatePaymentRequest:
        ...
        if "otp, action" in temp_dict.keys():
            return cls.from_dict(json.loads(json_str))
        return None
...
OrderAuthenticatePaymentRequest.model_rebuild()
